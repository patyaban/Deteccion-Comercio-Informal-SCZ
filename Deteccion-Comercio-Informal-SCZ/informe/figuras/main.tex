...
sentadas (\texttt{person\_sitting}) junto con seis clases de contexto vial y peatonal.
...
relevamiento y el sistema se limita a describir la ocupación observable del espacio: no
...
\texttt{cyclist} (5) conservan deliberadamente los índices y la semántica de KITTI,
de modo que la etapa intermedia sea informativa para ellas; las tres últimas
\texttt{street\_vendor} (6), \texttt{street\_stall} (7) y \texttt{motorbike} (8)
son propias del dominio.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Sesgo de captura y ambigüedad de anotación.} Todas las fotos son diurnas,
        ...
        frontera entre \texttt{street\_vendor} y \texttt{pedestrian} quien atiende
        frente a quien compra es una decisión de criterio que dos anotadores no aplican
        de forma perfectamente consistente.
...
régimen de datos. Segundo, el uso previsto pre-etiquetado por lotes y, eventualmente,
procesamiento de video exige inferencia barata: el modelo ejecuta en 19{,}2\,ms por
...
número de clases ($nc$: $8 \rightarrow 9$) y se congelan las 10 primeras capas el
...
de Pascal VOC a YOLO centro y dimensiones normalizados por el ancho y alto de la
imagen se implementó explícitamente y se verifica con una aserción en
...
resultado reportado. No se realizó barrido de hiperparámetros no había presupuesto de
cómputo ni un conjunto de validación lo bastante grande para que resultara
...
Épocas / \texttt{freeze} & 10 / --- & 80 / 10 & 80 / ---\\
...
---10 épocas están lejos de agotar KITTI---: lo relevante es que estos pesos ya codifican
...
7{,}5\,\% de confusiones de clase. Tres mecanismos concentran el 60\,\% de los errores:
personas ocluidas en escenas densas, puestos contiguos que el NMS suprime por
solapamiento mutuo, y la confusión \texttt{pedestrian}\,/\,\texttt{street\_vendor}. El
desglose completo está en el Anexo~\ref{anexo:errores}.
...
\texttt{street\_stall} clase que no existe en COCO ni en KITTI y de apariencia muy
...
modelo equivalente en la mitad del tiempo, aunque el \emph{checkpoint} final sigue
siendo el mejor de la corrida porque el mAP nunca deja de mejorar. La rama desde cero no
...
A ello se suman el sesgo de captura tomas diurnas, con buen clima y desde una
...
observables en la vía pública. No identifica personas no se aplicó reconocimiento
...
de pre-etiquetado detecta seis de cada diez puestos, en 313\,ms por imagen y sin
...
detecciones duplicadas (1{,}6\,\%). La Tabla~\ref{tab:modos} agrupa por mecanismo los
errores con un patrón identificable; el detalle caso por caso está en
\texttt{resultados/analisis/} del repositorio.
...
\midrule
Omisión en escena densa & 25 & Personas y puestos ocluidos entre sí o al fondo, con pocos píxeles. \texttt{pedestrian} (15) y \texttt{street\_vendor} (10) concentran la falla.\\
...
Omisión de clase vial & 9 & \texttt{motorbike}, \texttt{car} y \texttt{truck} ocluidos tras puestos, con soporte escaso en el conjunto local (3{,}6\,\%, 8{,}4\,\% y 1{,}5\,\%).\\
Detección espuria o duplicada & 11 & Activaciones sobre textura de mercadería y toldos, todas por debajo de 0{,}35 (8 casos); dos cajas sobre el mismo objeto con solapamiento inferior al umbral de NMS de 0{,}7 (3).\\
Confusiones de clase & 14 & \texttt{pedestrian} frente a \texttt{street\_vendor} (6 casos): misma silueta humana, y lo que las separa es la relación espacial con la mercadería, contexto que un detector de una etapa no modela. \texttt{person\_sitting} y \texttt{cyclist} (4) no tienen soporte para discriminar postura; carretillas y triciclos comparten estructura con \texttt{motorbike} y \texttt{truck} (2); \texttt{van} frente a \texttt{truck} es una frontera ambigua heredada de KITTI (2).\\
\bottomrule
\end{tabular}
\end{table}
