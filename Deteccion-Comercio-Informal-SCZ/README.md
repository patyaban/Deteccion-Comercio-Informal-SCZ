...
Por clase, el modelo secuencial alcanza mAP@0.5 de **0.552** en `street_stall` y **0.394** en `street_vendor`. El detalle completo y el análisis está en [`informe/`](informe/).
...
**Limitaciones conocidas** (el detalle está en la sección 3.4 del informe):
...
```bash
git clone https://github.com/USUARIO/deteccion-comercio-informal-scz.git
...
Entorno de referencia del resultado reportado: Python 3.12, PyTorch 2.11.0 + CUDA 12.8, Ultralytics 8.4.123, GPU NVIDIA Tesla T4 (Google Colab).
...
En `datos/muestra/` hay un puñado de imágenes de ejemplo versionadas en el repo, suficientes para correr el notebook de inferencia sin descargar nada.
...
### 6. Figuras y análisis de errores
...
El segundo comando exporta 15 aciertos y 15 fallos recortados, más `analisis_errores.csv` con una columna `hipotesis` para completar (requisito de la rúbrica, sección 6 del informe).
...
├── .gitignore
├── configs/
│   └── scz_data.yaml               # esquema de 9 clases (referencia)
├── datos/
...
- Ninguna de las dos ramas convergió: la métrica de validación seguía subiendo en la época 80. El régimen dominante es **subentrenamiento**, no sobreajuste.
